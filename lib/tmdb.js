...
class TMDBClient {
    ...
    /**
     * Formate un film pour Stremio
     *
     * @param {Object} movie - Film TMDB
     * @returns {Object} Meta Stremio
     * @private
     */
    _formatMovie(movie) {
        ...
        return {
            ...
            background: movie.backdrop_path
                ? `${TMDB_IMAGE_URL}/w1280${movie.backdrop_path}`
                : null,
    ...
    /**
     * Formate une série pour Stremio
     *
    ...
    _formatSeries(series) {
        ...
        return {
            ...
            poster: series.poster_path
                ? `${TMDB_IMAGE_URL}/w500${series.poster_path}`
                : null,
            background: series.backdrop_path
                ? `${TMDB_IMAGE_URL}/w1280${series.backdrop_path}`
                : null,
            description: series.overview,
            releaseInfo: series.first_air_date?.substring(0, 4),
            imdbRating: series.vote_average?.toFixed(1),
            year: series.first_air_date ? parseInt(series.first_air_date.substring(0, 4)) : null,
    ...
    async getTrendingMoviesDay(page = 1) {
        return cached(`trending_movies_day_${page}`, async () => {
            ...
            return data.results.map(m => this._formatMovie(m)).filter(Boolean);
        });
    }
    ...
    async getTrendingSeriesDay(page = 1) {
        return cached(`trending_series_day_${page}`, async () => {
            ...
            return data.results.map(s => this._formatSeries(s)).filter(Boolean);
        });
    }
